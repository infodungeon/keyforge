...
export function KeyPicker({ onInsert }: Props) {
  ...
  if (!data)
    return (
      <div className="w-80 p-8 text-slate-500 text-xs animate-pulse">
        Loading...
      </div>
    );

  const allKeys = Object.entries(data.categories).flatMap(([_, groups]) =>
    groups.flatMap((g) => g.items.map((k) => ({ label: k, group: g.label }))),
  );
          ...
          <div className="grid grid-cols-3 gap-2">
            {filtered.map((k, i) => (
              <button
                ...
                className="p-2 bg-slate-800 hover:bg-purple-600 hover:text-white rounded text-[10px] font-mono border border-slate-700 transition-colors relative hover:z-10"
              >
                {k.label}
              </button>
            ))}
            ...
            {data.tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-[3rem] py-2 rounded-lg flex flex-col items-center justify-center gap-1 transition-all ${activeTab === tab.id ? "bg-slate-800 text-purple-400 border border-slate-700" : "text-slate-500 hover:bg-slate-800/50 hover:text-slate-300"}`}
                >
                  <Icon size={16} />
