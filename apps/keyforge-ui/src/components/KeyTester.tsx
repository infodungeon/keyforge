...
export function KeyTester() {
  ...
  useEffect(() => {
    backend
      .getKeyboardGeometry("ansi_104")
      .then(setGeometry)
      .catch((e) => {
        console.error(e);
        setError(
          "Failed to load ANSI Layout. Please ensure standard keyboards are loaded.",
        );
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (!e.repeat) {
      setHistory((prev) =>
        [{ key: e.key, code: e.code }, ...prev].slice(0, 50),
      );
    }
  };
      ...
      <div className="h-24 shrink-0 border-t border-slate-800 pt-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-800">
          {history.map((h, i) => (
            <div
              key={i}
              className="flex flex-col items-center shrink-0 min-w-[3rem]"
            >
              <div
                className={`
