    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-center">
          <div>
            ...
            <div className="text-2xl font-black text-purple-400">
              {wpm} <span className="text-xs text-slate-600">WPM</span>
            </div>
          </div>
          <div>
            <div className="text-[10px] text-slate-500 uppercase font-bold">
        ...
        <div>
          ...
          <div className="flex items-center bg-slate-950 border border-slate-800 rounded-lg p-1">
            <button
              ...
              className="p-2 hover:bg-slate-800 text-slate-500 hover:text-white rounded transition-colors"
            >
              <Minus size={14} />
            </button>
      ...
      <Card className="bg-slate-900 border-slate-800 p-4 space-y-3">
        ...
        <Button
          ...
          onClick={() => {
            if (confirm("Reset all biometric progress? This cannot be undone."))
              resetData();
          }}
          icon={<Trash2 size={14} />}
        >
          ...
          {showWeights && (
            <div className="mt-3 space-y-1 max-h-40 overflow-y-auto custom-scrollbar pr-2">
              {Object.entries(weights).map(([k, v]) => (
                <div
                  key={k}
                  className="flex justify-between text-[10px] border-b border-slate-800/50 pb-1 last:border-0"
                >
