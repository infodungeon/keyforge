...
export class CoverageService {
  ...
  public getStats(): CoverageStats {
    ...
    this.targetBigrams.forEach((t) => {
      ...
      const saturation =
        Math.min(count, SATURATION_THRESHOLD) / SATURATION_THRESHOLD;
    ...
    return {
      totalSamples: Array.from(this.seenBigrams.values()).reduce(
        (a, b) => a + b,
        0,
      ),
      coveragePct,
      mostNeeded: this.targetBigrams
        ...
        .sort(
          (a, b) =>
            (this.seenBigrams.get(a) || 0) - (this.seenBigrams.get(b) || 0),
        ),
    };
  }
  ...
  public selectTargetedWords(pool: string[], count: number): string[] {
    ...
    const neededList =
      stats.mostNeeded.length > 0 ? stats.mostNeeded : this.targetBigrams;
