...
export function LayoutView({ isSyncing, onSync }: Props) {
  ...
  const insertToken = (token: string) => {
    const { selectedKeyIndex, isEditingKey, layoutString, maxKeys } =
      stateRef.current;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (selectedKeyIndex !== null) {
        ...
        if (e.key === "Escape") {
          e.preventDefault();
          if (isEditingKey) setIsEditingKey(false);
          else setSelectedKeyIndex(null);
          return;
        }
        if (e.key === "Enter") {
          e.preventDefault();
    ...
    <>
      ...
      >
        <div className="h-14 flex items-center px-6 border-b border-slate-800/50 justify-between bg-[#0B0F19]/90 backdrop-blur z-10">
          <div className="flex items-center gap-2">
            ...
            {selectedKeyIndex !== null && (
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded border transition-colors ${isEditingKey ? "text-white bg-blue-600 border-blue-500" : "text-blue-400 bg-blue-900/30 border-blue-800"}`}
              >
        ...
        >
          ...
          <div
            className="mt-8 w-full max-w-2xl flex gap-2"
          ...
          >
            <Input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && handleCommitInput(layoutString)
              }
            ...
            <Button
              ...
              onClick={() => handleCommitInput(layoutString)}
            />
          </div>
