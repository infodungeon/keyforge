...
impl Replica {
    ...
    pub(crate) fn calc_bigram_delta(
            ...
            unsafe {
                ...
                for i in start..end {
                    ...
                    if p_other != KEY_NOT_FOUND_U8 as usize {
                        // SAFETY: Logic guarantees p_other is a valid key index or NOT_FOUND
                        debug_assert!(
                            p_other < n,
        ...
        if char_a < 256 && char_b < 256 {
            unsafe {
                ...
                if freq_ab > 0.0 {
                    // SAFETY: idx_a and idx_b are guaranteed < n
            ...
            unsafe {
                ...
                for t in trigrams {
                    ...
                    if p1_old != KEY_NOT_FOUND_U8 as usize && p2_old != KEY_NOT_FOUND_U8 as usize {
                        // SAFETY: Validated logic in debug mode
                        ...
                        // Debug Assertions for the scary pointer math
                        debug_assert!(
                            p_c_old * n_sq + p1_old * n + p2_old
                                < self.scorer.trigram_cost_table.len()
                        );
    ...
    pub fn calc_delta(&self, idx_a: usize, idx_b: usize, trigram_limit: usize) -> (f32, f32) {
        ...
        unsafe {
            ...
            // Optimization: If delta is huge (very bad move), abort early
