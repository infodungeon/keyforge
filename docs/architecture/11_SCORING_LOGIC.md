...
To ensure perfect determinism across different hardware architectures and avoid floating-point accumulation errors, the core engine uses fixed-point arithmetic.
- **`SCORE_SCALE`**: 1,000,000.0
- **`Score` Newtype**: A transparent wrapper around `i64`.
- All physical distances and weights are scaled by `SCORE_SCALE` during compilation and stored as integers.
...
The engine evaluates a specific `Layout` (a mapping of physical indices to KeyCodes). The total score is the sum of three layers of analysis.
...
$$Score_{mono} = \sum_{char} Freq(char) \times (Effort(finger) + Distance(key, origin))$$
...
$$Score_{bi} = \sum_{pair} Freq(pair) \times Cost(key_1, key_2)$$
...
## 5. Verification (The Oracle)

The system maintains a `DeterministicScorer` (the Oracle) in `verify.rs`. This is a slow, non-optimized implementation that uses floating-point math and high-level logic.
- **Purpose**: All optimized kernel changes must pass a parity check against the Oracle.
- **Parity**: The Oracle and the Fast Engine must match within a 0.001% tolerance to account for the float-to-fixed conversion noise.
