...
pub fn generate_physics_svg(geo: &KeyboardGeometry) -> String {
    ...
    let _ = write!(svg, r#"<svg viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">"#);
    ...
    for key in &geo.keys {
        ...
        let _ = write!(svg, r##"<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="2" fill="{color}" stroke="#dee2e6" stroke-width="0.5" />"##);
        
        // Add label if present
        if !key.label.is_empty() {
            let _ = write!(svg, r##"<text x="{}" y="{}" font-family="sans-serif" font-size="3" fill="#495057" text-anchor="middle" alignment-baseline="middle">{}</text>"##,
                x + w / 2.0,
                y + h / 2.0,
                key.label
            );
