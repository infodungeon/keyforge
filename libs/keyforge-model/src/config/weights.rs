...
impl Validator for ScoringWeights {
    fn validate(&self) -> Result<(), String> {
        ...
        if self.get_penalty_sfb_base() > MAX_SAFE_WEIGHT || self.get_penalty_scissor() > MAX_SAFE_WEIGHT {
            return Err(format!("Weights cannot exceed {MAX_SAFE_WEIGHT:.0}"));
        }
        for (i, &w) in self.finger_penalty_scale.iter().enumerate() {
            if w < 0.0 { return Err(format!("finger_penalty_scale[{i}] cannot be negative")); }
        }
        Ok(())
    }
}
...
impl ScoringWeights {
    ...
    pub fn get_threshold_sfb_long_row_diff(&self) -> i8 { self.get_weight("threshold_sfb_long_row_diff", f32::from(DEFAULT_THRESHOLD_SFB_LONG_ROW_DIFF)) as i8 }
    /// Gets the row difference threshold for scissors.
    #[must_use] 
    #[allow(clippy::cast_possible_truncation)]
    pub fn get_threshold_scissor_row_diff(&self) -> i8 { self.get_weight("threshold_scissor_row_diff", f32::from(DEFAULT_THRESHOLD_SCISSOR_ROW_DIFF)) as i8 }
