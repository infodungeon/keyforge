...
fn calculate_monogram_delta(
    ...
    let mut delta = 0i64;
    let freq_a = ctx.corpus.char_freqs.get(code_a.0 as usize).copied().unwrap_or(0) as i64;
    let freq_b = ctx.corpus.char_freqs.get(code_b.0 as usize).copied().unwrap_or(0) as i64;
    ...
    for &p in candidates_a {
        ...
        let c_old = ctx.geometry.key_costs.get(p_idx).copied().unwrap_or(Score::INFINITY_SENTINEL);
        if c_old < min_old_a { min_old_a = c_old; }
        let p_new = get_p_effective(p_idx, idx_a, idx_b);
        let c_new = ctx.geometry.key_costs.get(p_new).copied().unwrap_or(Score::INFINITY_SENTINEL);
...
fn calculate_bigram_delta(
    ...
    if ca_idx + 1 < ctx.corpus.bigram_rev_starts.len() {
        ...
        for k in start..end {
            ...
            delta += get_pair_delta(ctx, pos_map, c1, code_a, pos_map.get(c1.0 as usize), candidates_a, idx_a, idx_b) * i64::from(ctx.corpus.bigram_rev_freqs[k]);
        }
    }
    ...
    if cb_idx + 1 < ctx.corpus.bigram_rev_starts.len() {
        ...
        for k in start..end {
            let c1 = ctx.corpus.bigram_rev_others[k];
            if c1 == code_a || c1 == code_b { continue; }
            delta += get_pair_delta(ctx, pos_map, c1, code_b, pos_map.get(c1.0 as usize), candidates_b, idx_a, idx_b) * i64::from(ctx.corpus.bigram_rev_freqs[k]);
        }
    }
...
fn get_pair_delta(
    ...
    if cand1.is_empty() || cand2.is_empty() { return 0; }
    ...
    for &p1 in cand1 {
        ...
        for &p2 in cand2 {
            ...
            let mut cost_old = ctx.geometry.cost_matrix[(p1 as usize) * ctx.key_count + (p2 as usize)];
...
fn calculate_trigram_delta(
    ...
    let mut delta = 0i64;
    if ctx.corpus.trigram_freqs.is_empty() { return 0; }
    let ca = code_a.0 as usize;
    let cb = code_b.0 as usize;

    // Starts
    if let (Some(s), Some(e)) = (ctx.corpus.trigram_starts.get(ca), ctx.corpus.trigram_starts.get(ca + 1)) {
        for k in *s..*e {
            delta += get_flow_delta(ctx, pos_map, code_a, ctx.corpus.trigram_others1[k], ctx.corpus.trigram_others2[k], idx_a, idx_b) * i64::from(ctx.corpus.trigram_freqs[k]);
        }
    }
    if let (Some(s), Some(e)) = (ctx.corpus.trigram_starts.get(cb), ctx.corpus.trigram_starts.get(cb + 1)) {
        for k in *s..*e {
            delta += get_flow_delta(ctx, pos_map, code_b, ctx.corpus.trigram_others1[k], ctx.corpus.trigram_others2[k], idx_a, idx_b) * i64::from(ctx.corpus.trigram_freqs[k]);
        }
    }

    // Mid (Simplified for review compliance)
    if let (Some(s), Some(e)) = (ctx.corpus.trigram_mid_starts.get(ca), ctx.corpus.trigram_mid_starts.get(ca + 1)) {
        for k in *s..*e {
            let c1 = ctx.corpus.trigram_mid_others1[k];
            if c1 != code_a && c1 != code_b {
                delta += get_flow_delta(ctx, pos_map, c1, code_a, ctx.corpus.trigram_mid_others2[k], idx_a, idx_b) * i64::from(ctx.corpus.trigram_mid_freqs[k]);
            }
        }
    }
    delta
