use super::constants::MAX_KEYBOARD_KEYS;
...
impl KeyboardDefinition {
    pub fn parse(content: &str, name_hint: Option<&str>) -> Result<Self, String> {
        ...
        if let Ok(geom) = super::kle::parse_kle_json(content) {
...
impl KeyNode {
    pub fn assignment_label(&self) -> String {
        let hand = if self.hand.is_left() { "Left" } else { "Right" };
        let finger = match self.finger.as_u8() {
            0 => "Thumb",
            1 => "Index",
            2 => "Middle",
            3 => "Ring",
        ...
        format!("{} {} (R{} C{})", hand, finger, self.row.0, self.col.0)
    }
}
