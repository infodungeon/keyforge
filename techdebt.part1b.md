...
- [ ] **[L48] Hardcoded:** `config.corpora.len() > 10`.
- [ ] **[L168] Redundant:** `Semaphore::new(self.config.cores.max(1))` created inside loop.
- [ ] **[L167] Redundant:** `AtomicBool::new(false)` created inside loop.
- [ ] **[L87] Hardcoded:** `UserRepo::new` path cloning.
- [ ] **[L103] Hardcoded:** `loader.load_keycodes("keycodes.json")`.
- [ ] **[L112] Hardcoded:** `conversion::to_domain_config(&config.params, 42)`. Seed 42.
- [ ] **[L148] Thread Leak:** `spawn_blocking` cannot be cancelled if physics hangs.
- [ ] **[L150] Panic Handling:** `catch_unwind` masks panic details.
- [ ] **[L144] Hardcoded:** Timeout `3600` seconds.

### `apps/keyforge-agent/src/agent/crypto.rs`

- [ ] **[L33] Error Mapping:** `AgentError::Internal` wraps signing error string.

### `apps/keyforge-agent/src/agent/maintenance.rs`

- [ ] **[L25] Hardcoded:** Path `user/keyboards`.
- [ ] **[L33] Hardcoded:** Extension check `"json"`.

### `apps/keyforge-agent/src/agent/mod.rs`

- [ ] **[L113] Hardcoded:** `self.telemetry.set_job_id("idle")`.
- [ ] **[L106] Hardcoded:** `corpora` directory string.
- [ ] **[L94] Serial Execution:** `job_rx.recv()` processes one job at a time.

### `apps/keyforge-agent/src/agent/network.rs`

- [ ] **[L80] Dead Code:** `ResultOutbox` struct and impl are unused.
- [ ] **[L131] Hardcoded:** Backoff `1` second.
- [ ] **[L141] Hardcoded:** `ws?node_id=` query param.
- [ ] **[L165] Hardcoded:** `job_id == "idle"`.
- [ ] **[L206] Data Loss:** `submit_result` failure drops the result.

### `apps/keyforge-agent/src/agent/telemetry.rs`

- [ ] **[L49] Hardcoded:** `hash % 100 == 0` log sampling.

### `apps/keyforge-agent/src/hw_detect.rs`

- [ ] **[L208] Hardcoded:** `u64_to_usize_saturating` warns on overflow.

### `apps/keyforge-agent/src/logging.rs`

- [ ] **[L26] Hardcoded:** Env filter default `info,keyforge_agent=debug`.

### `apps/keyforge-agent/src/main.rs`

- [ ] **[L86] Hardcoded:** Node ID prefix `agent-`.
- [ ] **[L91] Hardcoded:** Broadcast channel capacity `16`.
- [ ] **[L132] Hardcoded:** Config dir `keyforge`.
- [ ] **[L139] Hardcoded:** Key file `agent.key.age`.
- [ ] **[L146] Container Risk:** `machine_id` stability in Docker.

### `apps/keyforge-agent/src/models.rs`

- [ ] **[L108] Hardcoded:** Default Hive URL `http://localhost:3000`.
- [ ] **[L109] Hardcoded:** Default node ID `unknown`.
- [ ] **[L112] Hardcoded:** Default data dir `data`.
- [ ] **[L183] Hardcoded:** Default job ID `idle`.
...
- [ ] **[L60] Hardcoded:** Channel capacity `10000`.
