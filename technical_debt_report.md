...
- **[src-tauri/src/commands/arena.rs:L121]** **Panic Risk**: `unwrap()` used during sort comparison (same issue as CLI).
...
- **[context/SessionContext.tsx]** **sloppy**: Leftover `console.log("DEBUG: ...")` statements polluting the console.
...
### `libs/keyforge-model`
- **[config.rs:L215-L296]** **STRUCTURAL DEBT**: `ScoringWeights` is a monolithic struct. Adding a new scoring metric (e.g., "staircase penalty") requires manual field additions across `Model`, `Protocol`, `Persistence`, `CLI`, and `UI`.
- **[config.rs:L293, L339, L415]** **STRINGLY-TYPED CONFIG**: Penalties like `finger_penalty_scale` are stored as comma-separated strings instead of numeric arrays, forcing manual parsing every time the weights are used.
- **[parsing.rs:L73-L131]** **BRITTLE KEYMAP PARSING**: Hardcoded regex and logic for matching QMK (`MO(1)`) and ZMK (`&mo 1`) styles. Adding support for a new firmware (like BlueMicro) would require invasive changes to the model.
- **[types.rs:L150-L189]** Implicit Scaling: `Score` uses a custom fixed-point `i64` implementation that converts from `f32` on every creation; risk of precision drift in high-iteration optimization loops.
- **[constants.rs:L92]** **HARDCODED EFFORT**: Defaults for finger efforts are baked into the binary, preventing users from defining their own physical "effort map" without a recompile.
- **[job.rs:L99]** Legacy Panic: Some helper methods like `from_parts` (now deprecated) were prone to panics on serialization failure.
...
- **[lib.rs:L127, L182]** Hex length fragility: strictly requires exactly 64 characters for Ed25519 keys; fails on any variation or prefix.
- **[lib.rs:L102]** Hardcoded capacity: `32 + 32 + 4 + 8 + 8` manually calculated for the hashing payload; brittle if structural types change.
...
- [RESOLVED] **[qmk.rs:L31-L32]** Arbitrary hardcoded limits: `1MB` output size and `200` keys.
- [RESOLVED] **[qmk.rs:L80-L91]** Manual mapping of mod names.
- [RESOLVED] **[qmk.rs:L24-L29]** Brittle sanitization.
- [RESOLVED] **[via.rs:L55]** Hardcoded 1-layer limitation.

### `libs/keyforge-compute` & `keyforge-core`
- [RESOLVED] **[builder.rs:L77]** Hardcoded default seed: `42`.
- [RESOLVED] **[builder.rs:L90]** Magic numbers / Fake telemetry.
- [RESOLVED] **[loader.rs:L89-L106]** **CODE DUPLICATION**: `RawCostData::resolve` duplicate.
