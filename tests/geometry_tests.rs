...
fn test_custom_geometry_distance() {
    ...
    let scale = 1.0;
    let dist = get_geo_dist(&geom, 0, 1, scale);
...
fn test_custom_finger_assignment_sfb() {
    ...
    let geom = make_2key_geom(k1, k2);
    let result = analyze_interaction(&geom, 0, 1);
...
fn test_custom_hand_assignment() {
    ...
    let analysis = analyze_interaction(&geom, 0, 1);
    assert!(!analysis.is_same_hand, "Hand assignment failed");
}

#[test]
fn test_non_sfb_lateral_stretch() {
    // Test the specific case of a Lateral Stretch that is NOT an SFB
    // e.g. Index finger adjacent to Middle finger in the stretch zone
    // Same Row, Adjacent Col, Stretch is True, Different Fingers.
    let k1 = KeyNode {
        hand: 0,
        finger: 1,
        row: 0,
