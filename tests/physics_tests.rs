...
// Note: In the new physics, 'is_lateral_stretch' tracks non-SFB lateral adjacency
#[rstest]
...
// Note: F->G is an SFB, so is_lateral_stretch is FALSE (it's an SFB Lat Step)
// We need distinct fingers for is_lateral_stretch test.
// Standard layout doesn't have two fingers on same row/adjacent col in stretch zone
// easily without crossing hands or redefining fingers.
// However, we can check the logic for H -> G if H was Right Hand and G was Left? No.
// Let's test the logic: Same Row, Adjacent Col, Stretch, DIFFERENT FINGERS.
// On standard board: T(4) is Index. R(3) is Index. R->T is SFB.
// We need to fake a geometry to test non-SFB lateral stretch effectively,
// or acknowledge standard layout implies SFB for laterals usually.
// Let's use a custom geometry test for this in geometry_tests.rs
// Here we mostly verify standard behavior.
